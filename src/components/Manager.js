...
const Manager = () => {
  ...
  const handleAddClick = async () => {
    const isAnyOptionEmpty = dropdownMenus.some(
    ...
    if (
      isAnyOptionEmpty ||
      partsProduced === "" ||
      (timeRequired === "custom" && customTime === "")
    ) {
      alert(
        "Please select an option for all fields and enter values for Parts Produced and Time Required"
      );
      return;
    }
