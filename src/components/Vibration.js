...
function Vibration() {
  ...
  const fetchPast24HVibs = async (machineId) => {
    ...
    console.log(vib);
    setPast24HVibsData((prevData) => [
      ...prevData,
      { machineId, vib, vib1, vib2 },
    ]);
  };
  ...
  const fetchPast24HVibs1 = async (machine) => {
    ...
    const temps = response.data;
    console.log(
      `Past 24-hour temperatures for DeviceId ${machine} have been received!`
    );
    console.log(temps);
    let data1 = [["Time", "X1"]];
    let data2 = [["Time", "Y1"]];
